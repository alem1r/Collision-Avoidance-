...
p_min(0) = cloud.points[540].x; //la lunghezza di cloudpoints è 1080, la metà è 540 che sta davanti a me. (solo per inizializzare)
...
const float min_detect = cloud.points.size()/2-200;
const float max_detect = cloud.points.size()/2+200;

for(int i=min_detect;i<=max_detect;i++){
  Eigen::Vector2f p;

  p(0) = cloud.points[i].x; 
  p(1) = cloud.points[i].y;  

  auto obstacle_pos_actual = laser_transform * p;
...
if(obstacle_dist < 0.5 && vel_received.linear.x > 0 ){
  ...
  if(obstacle_pos(1) > 0)
    msg_send.angular.z = - force_intens; 
